...
fn mmc3_long_tail_variants_258_266_267_291_321_334_361_364_366_367_370_373_use_outer_registers_and_dip_reads(
    ...
    let mut m364 = Mapper::new(364, 256, 512, Mirroring::Vertical, 0).expect("mapper 364");
    assert!(m364.watches_ppu_bus());
    assert!(m364.write_low_register(0x6000, 0x72));
    assert!(!m364.low_register_write_falls_through(0x6000));
    m364.write_register(0x8000, 0x06);
    m364.write_register(0x8001, 0x3F);
    m364.write_register(0x8000, 0x02);
    m364.write_register(0x8001, 0xFF);
    assert_eq!(m364.prg_index(0x8004), 0x2F * 0x2000 + 4);
    assert_eq!(m364.chr_index(0x1004), 0x17F * 0x400 + 4);
    m364.reset(true);
    assert_eq!(m364.prg_index(0x8004), 0x20 * 0x2000 + 4);

    let mut m366 = Mapper::new(366, 128, 256, Mirroring::Vertical, 0).expect("mapper 366");
    assert!(m366.watches_ppu_bus());
    assert!(m366.write_low_register(0x6030, 0x00));
    assert!(m366.low_register_write_falls_through(0x6030));
    m366.write_register(0x8000, 0x06);
    m366.write_register(0x8001, 0x2A);
    m366.write_register(0x8000, 0x02);
    m366.write_register(0x8001, 0xD5);
    assert_eq!(m366.prg_index(0x8004), 0x3A * 0x2000 + 4);
    assert_eq!(m366.chr_index(0x1004), 0x1D5 * 0x400 + 4);
    assert!(m366.write_low_register(0x7080, 0x00));
    assert!(m366.write_low_register(0x7010, 0x00));
    ...
    let mut m373 = Mapper::new(373, 512, 512, Mirroring::Vertical, 0).expect("mapper 373");
    assert!(m373.watches_ppu_bus());
    m373.write_register(0x8000, 0x06);
    m373.write_register(0x8001, 0x2A);
    m373.write_register(0x8000, 0x07);
    m373.write_register(0x8001, 0x2D);
    assert!(m373.write_low_register(0x6000, 0x34));
    assert!(m373.write_low_register(0x6000, 0x20));
    assert!(m373.write_low_register(0x6000, 0x60));
    assert!(m373.write_low_register(0x6000, 0x3C));
    assert_eq!(m373.prg_index(0x8004), 0x122 * 0x2000 + 4);
