...
- `Mapper301::write_register()` / `write_expansion()` / `reset()` 对应 FCEUmm `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/301.c:24-58`；本项目在 `fc-core/src/mapper/basic/multicart.rs:1734-1799` 按 `asic_latch` 语义保存 latch data、`$5000-$5FFF` outer register、reset half toggle、PRG16 fixed-high 与 bit2 mirroring。
- `Mapper340::write_register()` / `prg_index()` / `chr_write()` 对应 FCEUmm `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/340.c:24-50`；本项目在 `fc-core/src/mapper/basic/multicart.rs:1801-1878` 表达 address latch、NROM/UNROM mode、submapper 0/1 mirroring 差异，以及 NROM mode CHR-RAM 写保护。
- `Mapper341::write_register()` / `prg_index()` / `chr_index()` 对应 FCEUmm `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/341.c:24-39`；本项目在 `fc-core/src/mapper/basic/multicart.rs:1880-1933` 表达 address latch PRG32/CHR8、submapper 1 的 `A7..A4 == $A` latch trap 与 mirroring bit 差异。
- `Mapper343::write_register()` / `write_expansion()` / `reset()` 对应 FCEUmm `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/343.c:24-52`；本项目在 `fc-core/src/mapper/basic/multicart.rs:1935-1988` 表达 latch data PRG32、`$5000-$5FFF` mirroring register、CHR8 fixed 0 和 reset clear。

## 以后替换时的删除边界

- 先替换 `fc-core/src/mapper/basic/unlicensed.rs:1-884`。
- 同批替换 `fc-core/src/mapper/basic/latch/discrete.rs` 里 Mapper 29 / 36 / 72 / 79 / 92 / 99 / 122 的新增段、`fc-core/src/mapper/basic/latch/sachen.rs` 里 Sachen 133 / 137 / 141 / 146 / 148 / 149 / 150 / 243 的新增段、`fc-core/src/mapper/basic/core.rs` 里 ColorDreams/Mapper144 的扩展段、`fc-core/src/mapper/basic/taito.rs` 里 Mapper 80 / 207 / 82 的新增段，以及 `fc-core/src/mapper/basic/multicart.rs` 里 Mapper 51 / 59 / 63 / 128 / 201 / 217 / 221 / 228 / 236 / 237 / 239 / 255 / 265 / 277 / 280 / 283 / 301 / 340 / 341 / 343 的新增段。
