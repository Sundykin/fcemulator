...
- `Mmc3::new_267()` / `Mmc3OuterBank::Mapper267` 对应 FCEUmm `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/267.c:26-62`；本项目在 `fc-core/src/mapper/mmc3/state.rs:118-120`、`constructors.rs:426-430`、`mmc3.rs:679-682,825-828,1537-1541,1949-1952` 表达 JY-119 outer bank 公式、低区一次性锁存、bit7 锁定后继续消费写但不更新寄存器，以及标准 MMC3 reset。Mesen2 本地 mapper factory 无 267 专用实现。
- `Mmc3::new_291()` / `Mmc3OuterBank::Mapper291` 对应 FCEUmm `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/291.c:24-59`；本项目在 `fc-core/src/mapper/mmc3/state.rs:121-123`、`constructors.rs:433-437`、`mmc3.rs:683-689,829,1545-1548,1949-1952` 表达 CHR bit8 扩展、普通 MMC3 PRG mode、bit5 PRG32 forced mode、低区寄存器写和标准 MMC3 reset。Mesen2 本地 mapper factory 无 291 专用实现。
- `Mmc3::new_321()` / `Mmc3OuterBank::Mapper321` 对应 FCEUmm `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/321.c:26-56`，并用 `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/asic_mmc3.c:71-82` 确认 AX5202P WRAM always-enabled/fall-through 语义；本项目在 `fc-core/src/mapper/mmc3/state.rs:124-126`、`constructors.rs:440-444`、`mmc3.rs:691-696,830,1549-1552,1571,1953-1956` 表达 PRG32 forced mode、普通 MMC3 PRG/CHR outer OR、低区写 fall-through 与 reset。
- `Mmc3::new_334()` / `Mmc3OuterBank::Mapper334` 对应 FCEUmm `/Users/sunmeng/workspace/fc/libretro-fceumm/src/boards/334.c:26-64`；本项目在 `fc-core/src/mapper/mmc3/state.rs:127-130`、`constructors.rs:447-451`、`mmc3.rs:698,831,1553-1558,1607-1629,1957-1960` 表达 PRG32 latch、低区偶地址写、open-bus/DIP 读和 reset DIP increment。Mesen2 `MapperFactory.cpp:574,586` 仅保留 321/334 注释占位，无可复刻实现。
...
- 同批替换 `fc-core/src/mapper/basic/discrete.rs` 里 Mapper 181 / 185 / 186 / 188 / 193 / 218 的新增段，以及 `fc-core/src/mapper/mmc3.rs` / `fc-core/src/mapper/mmc3/state.rs` / `fc-core/src/mapper/mmc3/constructors.rs` / `fc-core/src/mapper/mmc3/tests.rs` 里 Mapper14 / Mapper126 / Mapper176 / Mapper182 / Mapper187 / Mapper189 / Mapper191 / Mapper196 / Mapper197 / Mapper198 / Mapper199 / Mapper208 / Mapper215 / Mapper223 / Mapper224 / Mapper238 / Mapper245 / Mapper254 / Mapper267 / Mapper291 / Mapper321 / Mapper334 的 `Mmc3OuterBank` / `Mmc3ChrLayout` 分支、构造、低区写、扩展区读写、低区读、reset 和测试段。
